...
st.set_page_config(page_title="🔄 UPC Merge Tool (Category Fallback)", layout="wide")
st.title("🔄 UPC Merge Tool (with Category Hierarchy Detection)")

st.markdown("""
✅ Features:
- Smart header detection (first 5 rows)
- Multi-tab Excel support
- Flexible mapping:
  - `title` or `description` → `description`
  - `gtin`, `GTIN`, `UPC`, `barcode` → `barcode`
  - `brand` → `bh2Brand`
  - Unnamed `category` column with `>` is auto-split into `ch1Department`, `ch2Category`, `ch3Segment`
- Size & count parsing
- Merges only new UPCs into Partner Dashboard file
""")
...
if upc_file and partner_file:
    ...
    if not desc_col:
        desc_col = st.selectbox("⚠️ Select the description column:", options=columns)
    ...
    if not upc_col:
        upc_col = st.selectbox("⚠️ Select the barcode column:", options=columns)

    brand_col = 'brand' if 'brand' in columns else None

    # Detect any column with ">" values for fallback category parsing
    cat_col = None
    for col in columns:
    ...
    st.write(f"📦 Category Source Column: `{cat_col}`")

    if not desc_col or not upc_col:
        st.error("❌ You must select both a description and barcode column to continue.")
    else:
        if st.button("🚀 Merge & Export"):
            upc_df[upc_col] = (
                upc_df[upc_col]
                .astype(str)
            ...
            st.download_button(
                ...
                data=output,
                file_name="merged_with_category_parsing.xlsx",
